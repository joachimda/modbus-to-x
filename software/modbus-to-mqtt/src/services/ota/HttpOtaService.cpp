...
void HttpOtaService::loop() {
    ...
    if (!s_forceCheck && !timeToCheck) return;
    if (WiFiClass::status() != WL_CONNECTED) return;

    String err;
    bool available = false;
    String version;
    (void)checkForUpdate(err, available, version);
}
...
bool HttpOtaService::processManifestAndMaybeUpdate(const String& json) {
    ...
    if (!verifyManifestSignature(device, version, appUrl, appSha256, fsLabel, fsUrl, fsSha256, sig, kid)) {
